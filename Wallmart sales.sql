...
SELECT SUM(Quantity) AS qty, `Product line` FROM sales
...
SELECT `Product line`, SUM(total) as total_revenue FROM sales
GROUP BY `Product line`
...
SELECT `Product line`, AVG(`Tax 5%`) as avg_tax FROM sales
GROUP BY `Product line`
...
SELECT gender,`Product line`, COUNT(gender) AS total_cnt FROM sales
GROUP BY gender, `Product line`
ORDER BY total_cnt DESC;

-- What is the average rating of each product line
SELECT ROUND(AVG(rating), 2) as avg_rating, `Product line` FROM sales
GROUP BY `Product line`
...
SELECT DISTINCT payment FROM sales;-- In which city is each branch?
SELECT DISTINCT city,branch FROM sales;
...
SELECT `Customer type`,count(*) as count FROM sales
...
SELECT city, ROUND(AVG(`Tax 5%`), 2) AS avg_tax_pct FROM sales
GROUP BY city 
ORDER BY avg_tax_pct DESC;

-- Which customer type pays the most in VAT?
SELECT `Customer type`, AVG(`Tax 5%`) AS total_tax FROM sales
GROUP BY `Customer type`
ORDER BY total_tax;
